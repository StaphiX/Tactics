...
public class FSprite : FFacetElementNode
{
	...
	protected Rect _cropRect = new Rect(0,0,1.0f,1.0f); //crop rect is the part of the _localRect to draw
	...
	override public void PopulateRenderLayer()
	{
		if(_isOnStage && _firstFacetIndex != -1) 
		{
			...
			float useLeft = Mathf.Clamp(_cropRect.x, 0.0f, 1.0f);
			float useRight = Mathf.Clamp(_cropRect.width, useLeft, 1.0f);
			float useBottom = Mathf.Clamp(_cropRect.y, 0.0f, 1.0f);
			float useTop = Mathf.Clamp(_cropRect.height, useBottom, 1.0f);
...
//			uvs[vertexIndex3] = _element.uvBottomLeft;
	...
	virtual public void SetDimensions(float fX, float fY, float fW, float fH)
	{
		SetPosition(new Vector2(fX, fY));
    	width = fW;
    	height = fH;
	}

	virtual public void SetDimensions(float fW, float fH)
	{
		width = fW;
		height = fH;
	}
